program atmosphere_test 
    ...
    real :: temp_k, pressure_N_per_m2, density_kg_per_m3, dyn_viscosity_pa_sec, sos_m_per_sec
    real :: res(7)
    integer :: i, io_unit
    ...
    ! BUILD THE HEADER
    write(io_unit,'(8X,A21,5X,A24,2X,A14,12X,A15,A26,11X,A19,A20,A25)') &
        "Geometric_Altitude[m]", "Geopotential_Altitude[m]", "Temperature[K]", &
        "Pressure[N/m^2]", "Density[kg/m^3]","Speed_of_Sound[m/s]","Dynamic Viscosity"
    ...
    do i = 0, 90000, 5000
        ...
        res = (/geometric_altitude_m, geopotential_altitude_m, temp_k, pressure_N_per_m2, density_kg_per_m3, sos_m_per_sec, dyn_viscosity_pa_sec/)

        write(io_unit,'(7ES26.12)') res
