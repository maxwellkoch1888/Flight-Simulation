...
FONT_SIZE = 30
FONT_PATH = r"C:\Windows\Fonts\arial.ttf"  # Change as needed
# -----------------------------------------------------------
...
for angle in range(DEG_MIN, DEG_MAX + 1, 5):
    ...
    if angle == 0:
        # Draw a slightly longer line in the center
        draw.line((100, y, 300, y), fill=COLOR, width=2)
        continue
    ...
    if angle % 10 == 0:
        ...
        draw.line((x_center, y, x_center + SHORT_SEG, y), fill=COLOR, width=3)
        # Right long segment
        x_right = x_center + SHORT_SEG + GAP
        draw.line((x_right, y, x_right + LONG_SEG, y), fill=COLOR, width=3)
    else:
        # Minor tick every 5 degrees
        draw.line((140, y, 260, y), fill=COLOR, width=2)
